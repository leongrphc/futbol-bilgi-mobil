...
class WikidataClient:
    ...
    def _text_values(value: Any) -> list[str]:
        """Extract text values from both Wikibase formatversion=1 and 2 shapes.

        Depending on the API response format, labels/aliases can be returned as
        strings, {"value": ...} dictionaries, language-keyed dictionaries, or
        lists containing any of those forms. Unknown shapes are ignored instead
        of aborting an entire club import.
        """
        if value is None:
            return []
        ...
        if isinstance(value, list):
            output: list[str] = []
            for item in value:
                output.extend(WikidataClient._text_values(item))
            return output
        ...
        for start in range(0, len(qids), 50):
            ...
            for entity in entities:
                if not isinstance(entity, dict):
                    continue
                qid = entity.get("id")
                if not isinstance(qid, str) or qid not in result:
                    continue
                values = self._text_values(entity.get("labels"))
                values.extend(self._text_values(entity.get("aliases")))
                result[qid] = list(dict.fromkeys(value.strip() for value in values if value.strip()))
            time.sleep(self.pause_seconds)
        return result
