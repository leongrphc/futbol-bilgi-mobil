...
export interface MatchTicket { playerId: string; matchId: string; mode?: "bot" | "quick" | undefined }
...
export async function verifyMatchTicket(token: string, matchId: string, secret: string): Promise<MatchTicket> {
  ...
  return { playerId: payload.sub, matchId, mode: payload.mode === "bot" || payload.mode === "quick" ? payload.mode : undefined };
}
