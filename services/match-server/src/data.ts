...
export async function createMatchData(env: Env, requestedVersion?: string): Promise<MatchData> {
  ...
  const bootstrap = await rpc<{ football_data_version_id: string; clubs: Club[] }>(env, "get_match_bootstrap", { pool_size: 200, requested_version: requestedVersion ?? null });
  if (!bootstrap.football_data_version_id || bootstrap.clubs.length < 2) throw new Error("NO_ACTIVE_FOOTBALL_DATA");
  return {
